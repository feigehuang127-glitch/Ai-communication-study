...
ALTER TABLE users
  ...
  ADD COLUMN ai_sensitivity ENUM('low','medium','high') DEFAULT 'medium';

-- 扩展 questions 表
ALTER TABLE questions
  ADD COLUMN college ENUM('ai','comm') DEFAULT 'comm',
  ADD COLUMN tags JSON DEFAULT NULL;

-- 扩展 wrong_questions 表
ALTER TABLE wrong_questions
  ADD COLUMN college ENUM('ai','comm') DEFAULT NULL,
  ADD COLUMN tag VARCHAR(50) DEFAULT NULL;

-- 扩展 game_history 表
ALTER TABLE game_history
  ADD COLUMN college ENUM('ai','comm') DEFAULT 'comm',
  ADD COLUMN mode ENUM('pve','pvp','daily','practice') DEFAULT 'pve';
